...
def sync() -> None:
    ...
    try:
        print("=" * 60)
        print("🚀 MotherDuck Sync – Monitor Pipeline")
        print("=" * 60)
        ...
        if rows_read == 0:
            print("⚠️  No rows found – ensure the monitor_ingest step ran first.")
            return

        # ── Step 1.5: Silver Layer Cleaning ──────────────────────────────────
        # Monitors often have missing names or '0' prices from failed scrapers
        before = len(df)
        ...
        if dropped > 0:
            print(f"🧹 Filtered {dropped:,} invalid monitor records.")
